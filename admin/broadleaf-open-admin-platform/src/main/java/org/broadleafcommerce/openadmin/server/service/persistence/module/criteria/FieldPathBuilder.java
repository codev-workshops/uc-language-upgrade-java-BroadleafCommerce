...
public class FieldPathBuilder {
    ...
    public FieldPath getFieldPath(From root, String fullPropertyName) {
        ...
        for (String piece : pieces) {
            checkPiece: {
                if (j == 0 && isPluralAttribute(root, piece)) {
    ...
    public Path getPath(From root, FieldPath fieldPath, final CriteriaBuilder builder) {
        ...
        for (int i = 0; i < myFieldPath.getTargetPropertyPieces().size(); i++) {
            ...
            try {
                ...
                // use instead.
                SessionFactoryImplementor sfi = ((NodeBuilder) builder).getSessionFactory();
                Metamodel mm = sfi.getMetamodel();
                boolean found = false;

                Class<?>[] polyClasses = dynamicDaoHelper.getAllPolymorphicEntitiesFromCeiling(
                        path.getJavaType(), sfi, true, true);
    ...
    protected boolean isPluralAttribute(From<?, ?> from, String piece) {
        ManagedType<?> managedType = getManagedType(from);
        if (managedType != null) {
            try {
                return managedType.getAttribute(piece).isCollection();
            } catch (IllegalArgumentException e) {
                return false;
            }
        }
    ...
    protected ManagedType<?> getManagedType(From<?, ?> from) {
        Bindable<?> model = from.getModel();
        if (model instanceof ManagedType) {
            return (ManagedType<?>) model;
        }
        if (model instanceof SingularAttribute) {
            jakarta.persistence.metamodel.Type<?> type = ((SingularAttribute<?, ?>) model).getType();
            if (type instanceof ManagedType) {
                return (ManagedType<?>) type;
            }
        }
        if (model instanceof PluralAttribute) {
            jakarta.persistence.metamodel.Type<?> type = ((PluralAttribute<?, ?, ?>) model).getElementType();
            if (type instanceof ManagedType) {
                return (ManagedType<?>) type;
            }
        }
