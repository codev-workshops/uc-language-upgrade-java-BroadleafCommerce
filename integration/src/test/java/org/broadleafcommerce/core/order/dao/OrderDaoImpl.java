...
public class OrderDaoImpl implements OrderDao {
    ...
    public Order save(final Order order) {
        // Hibernate 5.6: cascade-merge on a managed entity replaces transient
        // collection elements with new managed copies, breaking object identity.
        // Track transient payments by reference number before merge so we can
        // sync their IDs afterward (tests rely on indexOf with the original object).
        java.util.Map<String, OrderPayment> transientPayments = null;
        if (em.contains(order) && order.getPayments() != null) {
            for (OrderPayment p : order.getPayments()) {
                if (p.getId() == null) {
                    if (transientPayments == null) {
                        transientPayments = new java.util.HashMap<>();
                    }
                    transientPayments.put(p.getReferenceNumber() + "|" + p.getType(), p);
                }
            }
        }
        Order response = em.merge(order);
        em.flush();
        // Copy assigned IDs back to original transient payment objects
        if (transientPayments != null && response.getPayments() != null) {
            for (OrderPayment p : response.getPayments()) {
                if (p.getId() != null) {
                    String key = p.getReferenceNumber() + "|" + p.getType();
                    OrderPayment orig = transientPayments.get(key);
                    if (orig != null && orig.getId() == null) {
                        ((OrderPaymentImpl) orig).setId(p.getId());
                    }
                }
            }
        }
    ...
    public Order readCartForCustomer(final Customer customer) {
        Order order = null;
        // Use a direct JPQL query with ORDER BY to ensure deterministic results
        // when multiple IN_PROCESS carts exist for the same customer (Hibernate 5.6
        // does not guarantee result order without explicit ordering)
        final TypedQuery<Order> query = em.createQuery(
                "SELECT o FROM " + OrderImpl.class.getName() + " o " +
                "WHERE o.customer.id = :customerId AND o.status = :orderStatus AND o.name IS NULL " +
                "ORDER BY o.id DESC", Order.class);
        query.setParameter("customerId", customer.getId());
        query.setParameter("orderStatus", OrderStatus.IN_PROCESS.getType());
        query.setHint(QueryHints.HINT_CACHEABLE, true);
        query.setHint(QueryHints.HINT_CACHE_REGION, "query.Order");
        final List<Order> temp = query.getResultList();
