...
public class VelocityMessageCreator extends MessageCreator {
    ...
    public String buildMessageBody(EmailInfo info, Map<String,Object> props) {
        ...
        if (props instanceof HashMap) {
            ...
            try {
                velocityEngine.mergeTemplate(info.getEmailTemplate(), info.getEncoding(),
                        new VelocityContext(propsCopy), writer);
            } catch (Exception e) {
                throw new RuntimeException("Unable to merge the velocity email template "
                        + info.getEmailTemplate(), e);
            }
