...
public class ResourceBundleProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext context, IProcessableElementTag tag,
        ...
        IModel replacement = modelFactory.createModel();
        ...
        if (getBundleEnabled()) {
            ...
            replacement.add(getElement(modelFactory, bundleUrl, async, defer));
        } else {
            for (String file : files) {
                ...
                replacement.add(getElement(modelFactory, value, async, defer));
            }
        }
    ...
    protected String getBundleUrl(ITemplateContext context, String bundleName) {
        ...
        String contextPath = webExchange.getRequest().getApplicationPath();
    ...
    protected IProcessableElementTag getScriptElement(IModelFactory modelFactory, String src, boolean async,
                                                      boolean defer) {
        ...
        return modelFactory.createStandaloneElementTag("script", attributes, null, false, true);
    }

    protected IProcessableElementTag getLinkElement(IModelFactory modelFactory, String src) {
        Map<String, String> attributes = new HashMap<String, String>();
        attributes.put("rel", "stylesheet");
        attributes.put("href", src);
        return modelFactory.createStandaloneElementTag("link", attributes, null, false, true);
    }

    protected IProcessableElementTag getElement(IModelFactory modelFactory, String src, boolean async, boolean defer) {
        if (src.contains(";")) {
