 ...
 * In Thymeleaf 3, this is now an IExpressionObjectFactory instead of extending SpelVariableExpressionEvaluator.
 * 
...
public class BroadleafVariableExpressionEvaluator implements IExpressionObjectFactory {
    ...
    public Object buildObject(IExpressionContext context, String expressionObjectName) {
        for (BroadleafVariableExpression expression : expressions) {
            if (!(expression instanceof NullBroadleafVariableExpression) && expression.getName().equals(expressionObjectName)) {
                return expression;
            }
        }
        return null;
