...
public class TransparentRedirectCreditCardFormProcessor extends AbstractElementModelProcessor {
    ...
    protected void doProcess(ITemplateContext context, IModel model, IElementModelStructureHandler structureHandler) {
        ...
        // Convert the <blc:transparent_credit_card_form> node to a normal <form> node, preserving attributes
        ...
        if (hiddenFields != null && !hiddenFields.isEmpty()) {
            for (String key : hiddenFields.keySet()) {
                Map<String, String> hiddenAttrs = new LinkedHashMap<String, String>();
                hiddenAttrs.put("type", "hidden");
                hiddenAttrs.put("name", key);
                hiddenAttrs.put("value", hiddenFields.get(key));
                IStandaloneElementTag hiddenNode = modelFactory.createStandaloneElementTag("input", hiddenAttrs, AttributeValueQuotes.DOUBLE, false, true);
                model.insert(model.size() - 1, hiddenNode);
            }
        }
    }
